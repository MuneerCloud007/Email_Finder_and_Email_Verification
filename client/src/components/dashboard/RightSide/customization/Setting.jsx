...
import {getUserData,updatePasswordSlice} from "../../../../features/slice/userSlice";
function Setting() {
    ...
    useEffect(()=>{
        if(user_Id){
            dispatch(getUserData({
                url:`/api/v1/user/getUserData/${user_Id}`,
                method:"get"
            }))
        }
  ...
  return (
    <div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <p class="text-gray-600">Your username is <strong>{userData.data["username"]}</strong></p>
      ...
      onClick={()=>{
        if(updatePassword["currentPassword"].length==0 || updatePassword["newPassword"].length==0){
            alert("Please fill all the fields")
        }
        dispatch(updatePasswordSlice({method:"put",url:`/api/v1/user/updatePassword/${user_Id}`,data:{
