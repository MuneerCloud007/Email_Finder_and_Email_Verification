...
import { Icon, Button, ClassHelper, useTranslation } from 'components/lib';
...
export function FileInput(props){
  ...
  const [error, setError] = useState(props.errorMessage || t('global.form.file.error.unselected'));
  ...
  function validate(files){
    ...
    if ((files.length + (props.value?.length || 0)) > props.max){
      ...
      setValid(false);
      setError(`${t('global.form.file.error.max')} ${props.max}`);
      return false;
      ...
      if (files.length){
        for (let i = 0; i < files.length; i++){
          ...
          if (props.accept?.length && !props.accept.includes(type)){
            ...
            setValid(false);            
            setError(`.${file.type.substring(file.type.indexOf('/')+1)} ${t('global.form.file.error.excluded')}`);
      ...
      <div className={ fileStyle }
        ...
        <div className={ Style.label }>
          ...
          { props.value?.length ?
            ...
            <div className={ Style.labelText }>{ t('global.form.file.dropzone') }</div>
          }
